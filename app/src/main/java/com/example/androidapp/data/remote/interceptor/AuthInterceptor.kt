...
class AuthInterceptor(
    ...
    override fun intercept(chain: Interceptor.Chain): Response {
        ...
        Timber.d("Request to $path needsAuth=$needsAuth")

        if (!needsAuth) {
            // No auth needed, proceed with original request
            Timber.d("Request to $path - No auth required, proceeding without token")
        ...
        if (accessToken.isNullOrEmpty()) {
            Timber.w("No access token available for $path, proceeding without token")
            return chain.proceed(originalRequest)
        }
